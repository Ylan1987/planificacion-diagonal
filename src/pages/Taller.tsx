...
const oneBase: Omit<OneShot,"draftDrive"|"draftPdf"|"pdf"|"driveLink"|"uploadedAt"|"_draftPdfFile">[] = [
  ...
  {"code":"taller_transito","name":"Plan de pedidos en transito.","resp":"Taller","start":addDays(PROJECT_START,14).toISOString(),"end":addDays(PROJECT_START,20).toISOString(),"details":["Cada estación de trabajo debe tener su lugar para los pedidos que entran y salen.\nDebe estar indicado claramente cual es el lugar de entrada y salida a cada maquina, así como perfectamente identificada cada maquina.\nNo podrá haber pedidos en transito fuera de esas áreas a menos que se este transportando en ese momento.\nLas áreas marcadas de tránsito deberán permanecer vacias en todo momento. Salvo durante un transito. \nLas mesas de trabajo son de trabajo, no de apoyo, por lo cual en caso de requerir una zona para entrada o salida, deberá tener otra mesa o establecer de manera clara cual es zona de entrada, cual es zona de trabajo y cual es zona de salida. \nTodos los trabajos que esten en una zona de salida o de entrada, deben estar perfectamente identificados, que trabajo es, que cliente es, que cantidad hay. \nLas mesas de trabajo deberán permanecer vacias de cualquier pedido con el que no se este trabajadno en ese momento. Por lo cual al cortar un pedido se deberá de mover el mismo a la zona de entrada. \nEntregable: Fotos documentales de cada máquina con su cartelería (nombre de máquina, zona de entrada y zona de salida), fotos documentales con las zonas de tránsito marcadas."]},
  {"code":"taller_deposito","name":"Plan de deposito","resp":"Taller","start":addDays(PROJECT_START,21).toISOString(),"end":addDays(PROJECT_START,27).toISOString(),"details":["El deposito deberá estar dividido en 3 grandes áreas. \n1. Materia prima \n2. Productos intermedios \n3. Productos terminados. \nLas 3 áreas deberán estar perfectamente identificadas (los cuartitos del taller son depositos, donde se guarda el rulo en el taller tambien, los placares de diseño también, el piso de arriba obviamente tambien) \nEn los depositos de bajo transito, deberá haber un listado de lo que hay (pedidos terminados deberá contener nombre de cliente, tipo de trabajo y cantidad). \nEn los intermedios si son de un cliente lo mismo que el anterior, pero agregarle que proceso le falta para estar pronto. Los productos intermedios genericos (papel cortado, interior estándar etc) y la materia prima, debera constar la cantidad que hay en ese deposito, su stock minimo y maximo. \nCada vez que se retira o ingresa parte del stock, deberá haber en la planilla un lugar para indicar la cantidad retirada, fecha y quien realizo el retiro. \nEn caso de dar quiebre al stock minimo, se deberá dar aviso al encargado de reponer. \nEntregable: Fotos documentales del deposito donde este claro que tipo de deposito es y las planillas con las cantidades y lugar para marcar entrada y salida"]},
  {"code":"taller_productividad","name":"Medición de productividad","resp":"Taller","start":addDays(PROJECT_START,7).toISOString(),"end":addDays(PROJECT_START,13).toISOString(),"details":["Usar la definición de distintos tipos de tareas marcados en el P4. \nEstablecer un lugar claro donde se pueda reportar: Fecha, hora de inicio, hora de fin, tarea realizada (del listado anterior), Numero de Pedido, Cantidad realizada. \nPoder sacar reportes de tiempos por tipo de tarea (divididos en tiempo de inicio (lectura de orden de trabajo, acomodo de la zona, busqueda de materiales, etc), tiempo de fin (limpieza, movimiento a la siguiente estacion etc), tiempo de trabajo). \nPoder sacar reportes por pedido Poder sacar reportes por persona."]},
...
const recurBase: RecurItem[] = [
  ...
  {"code":"taller_reporte_productividad","name":"Reporte por tipo de tarea, por persona, por pedido (Semanal)","freqDays":7,"nextAt":addDays(PROJECT_START,7).toISOString(),"history":[],"details":["Reporte de productividad, que ayude en la toma de decisiones de zafrales. \nQue sea uan herramienta fundamental en la presupuestación."]},
