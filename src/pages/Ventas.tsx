...
const oneBase: Omit<OneShot,"draftDrive"|"draftPdf"|"pdf"|"driveLink"|"uploadedAt"|"_draftPdfFile">[] = [
  {"code":"ventas_catalogo","name":"Catálogo de productos y variantes","resp":"Ventas","start":addDays(PROJECT_START,0).toISOString(),"end":addDays(PROJECT_START,13).toISOString(),"details":["Listado y PDFs base."]},
  {"code":"ventas_scripts","name":"Scripts de cotización y promesas de entrega","resp":"Ventas","start":addDays(PROJECT_START,14).toISOString(),"end":addDays(PROJECT_START,27).toISOString(),"details":["Guías con ejemplos."]},
  {"code":"ventas_sla","name":"SLA de comunicación con Planificación","resp":"Ventas + Planificación","start":addDays(PROJECT_START,28).toISOString(),"end":addDays(PROJECT_START,41).toISOString(),"details":["SLA y responsables."]}
];
...
export default function Ventas() {
  ...
  const LS_KEY = "delegacion-ui-v7-ventas";
  ...
  const submitOne = async (i: number) => {
    ...
    try {
      const key = "ventas/ones/" + t.code + "/" + Date.now() + "_" + t._draftPdfFile.name;
  ...
  const submitRecur = async (i: number) => {
    ...
    try {
      const key = "ventas/recs/" + r.code + "/" + Date.now() + "_" + r._draftPdfFile.name;
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      <header className="mx-auto max-w-[1400px] px-4 py-6 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-semibold">Ventas – Coordinación</h1>
      </header>
